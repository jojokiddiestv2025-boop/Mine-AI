...
const Sidebar: React.FC<SidebarProps> = ({ activeMode, onSelectMode }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLoad(prev => {
        ...
        return Math.max(5, Math.min(25, prev + jitter));
      });
    }, 3000);
  ...
  const menuItems = [
    { id: AppMode.CHAT, label: 'Mine Chat', subtitle: 'Search Grounding', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' },
    { id: AppMode.LITE, label: 'Fast Lite', subtitle: 'Snippet Engine', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
    { id: AppMode.IMAGE, label: 'Vision Pro', subtitle: 'Neural Edit', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
    { id: AppMode.COMPETITION, label: 'Elite Arena', subtitle: 'Championship Mode', icon: 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z' },
  ];

  return (
    <aside className="w-72 bg-slate-950/90 backdrop-blur-3xl border-r border-white/5 flex flex-col h-full shadow-2xl relative z-30">
      <div className="p-8 pb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
             ...
             <p className="text-[8px] text-slate-500 font-bold uppercase tracking-[0.4em]">Quota Protected v4.3</p>
          </div>
        </div>
      </div>
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full group flex items-center gap-4 px-5 py-4 rounded-2xl transition-all border ${
              activeMode === item.id
                ? 'bg-indigo-600/20 text-white border-indigo-500/40 shadow-[0_10px_30px_rgba(79,70,229,0.15)]' 
      ...
      <div className="p-6 border-t border-white/5 bg-slate-900/10">
        <div className="bg-slate-950/80 rounded-3xl p-5 border border-white/5 mb-4 shadow-inner">
          ...
          <div className="w-full bg-slate-900 h-2 rounded-full overflow-hidden">
            <div 
              className="bg-gradient-to-r from-indigo-600 to-emerald-500 h-full shadow-[0_0_15px_rgba(16,185,129,0.4)] transition-all duration-1000" 
              style={{ width: `${load}%` }}
            ></div>
          </div>
          <p className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest mt-2 italic">Optimized Load</p>
        </div>
        <div className="flex items-center gap-4 mb-6 px-2">
          <div className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden shadow-inner">
            <span className="text-xs font-black text-slate-400">{user?.email?.[0].toUpperCase()}</span>
          </div>
